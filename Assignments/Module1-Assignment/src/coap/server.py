...
class SensorResource(resource.ObservableResource):
    ...
    def __init__(self, line: str, sensor_type: str):
        ...
        asyncio.get_running_loop().create_task(self._update_loop())
...
class ActuatorResource(resource.Resource):
    ...
    async def render_put(self, request: Message) -> Message:
        ...
        try:
            ...
            return Message(code=Code.BAD_REQUEST, payload=_json({"error": "Malformed payload"}), content_format=50)

        if state not in {"ON", "OFF"}:
            return Message(code=Code.BAD_REQUEST, payload=_json({"error": "Invalid state"}), content_format=50)
...
class ManifestResource(resource.Resource):
    ...
    async def render_get(self, request: Message) -> Message:
        ...
        for index in range(80):
            entries.append({
                "device": f"sensor-{index:03d}",
                "line": f"line{1 if index % 2 == 0 else 2}",
                "type": ["temperature", "vibration", "power"][index % 3],
                "version": f"v{1 + index // 10}.{index % 10}.{(index * 7) % 10}",
                "checksum": f"sha256:{index:064x}",
                "url": f"https://firmware.smartfactory.local/releases/{index:03d}/firmware.bin",
                "size": 65536 + index * 1024,
        ...
        if len(payload) < 3072:
            payload = payload + b" " * (3072 - len(payload))
